...
void shabal_update(shabal_context_t* p_context, unsigned char* p_data, unsigned int p_offset, unsigned int p_length) {
	unsigned int* dataView = (unsigned int*)p_data;
...
//consume buffer_length bytes from ringbuffer
//assume buffer index and buffer length are multiple of sizeof(int)
void shabal_update_ring_buffer(shabal_context_t* p_context, unsigned char* p_data, unsigned int buffer_idx_bytes, unsigned int buffer_length_bytes) {
    unsigned int buffer_idx = buffer_idx_bytes / sizeof(unsigned int);
    unsigned int p_length = buffer_length_bytes;
    unsigned int buffer_length = buffer_length_bytes / sizeof(unsigned int);
    unsigned int p_offset = 0; // TODO: remove?
    ...
    for(unsigned int i = 0 ; i < numFullRounds ; ++i) {
        unsigned long base = (p_offset >> 2) + (i << 4);
        M0 = dataView[(buffer_idx + base) % buffer_length];
        M1 = dataView[(buffer_idx + base + 1) % buffer_length];
        M2 = dataView[(buffer_idx + base + 2) % buffer_length];
        M3 = dataView[(buffer_idx + base + 3) % buffer_length];
        M4 = dataView[(buffer_idx + base + 4) % buffer_length];
        M5 = dataView[(buffer_idx + base + 5) % buffer_length];
        M6 = dataView[(buffer_idx + base + 6) % buffer_length];
        M7 = dataView[(buffer_idx + base + 7) % buffer_length];
        M8 = dataView[(buffer_idx + base + 8) % buffer_length];
        M9 = dataView[(buffer_idx + base + 9) % buffer_length];
        MA = dataView[(buffer_idx + base + 10) % buffer_length];
        MB = dataView[(buffer_idx + base + 11) % buffer_length];
        MC = dataView[(buffer_idx + base + 12) % buffer_length];
        MD = dataView[(buffer_idx + base + 13) % buffer_length];
        ME = dataView[(buffer_idx + base + 14) % buffer_length];
        MF = dataView[(buffer_idx + base + 15) % buffer_length];
...
void shabal_update_global(shabal_context_t* p_context, __global unsigned char* p_data, unsigned int p_offset, unsigned int p_length) {
